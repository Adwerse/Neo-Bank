> 📖 This is the full engineering log — every sprint's decision, every
> mini-ADR, every manual verification. For the quick tour, start at
> [README.md](README.md).

# Neo-Bank

Register, fund an account with a test card, send money to another user, watch
the balance update live — a neobank you can run on your own laptop, built to
...
**[Demo script](DEMO.md)** · **[Load test report](https://claude.ai/code/artifact/b40504bd-656e-452a-bb32-3e4ec344bd26)** · **[Architecture diagram](docs/architecture.svg)** · Video walkthrough — recording soon

It's not a product — you can't open a real account on it, and no transfer
ever leaves the test environment. The point was to genuinely understand how
a banking system works: how a transfer can't push an account negative even
under concurrency, what happens when the database goes down mid-day, and how
to catch a fraudulent transfer before the money leaves. Every step answered
one concrete engineering question and ended in a measurement, not an
assumption — see [Engineering highlights](#engineering-highlights) below.
The list of what was deliberately not done, or done as a simulation, is also
part of the project, not an afterthought: see
...
cp .env.example .env       # Stripe placeholders — the stack starts fine without real keys,
                            # but deposits won't actually go through without them
docker compose up -d
```

That's the whole backend: 17 containers — 7 Go services, a 3-node Postgres
cluster (Patroni + etcd + HAProxy), Kafka, Redis, MinIO, Jaeger, Mailpit. No
Stripe key? Registration, transfers, fraud checks, the live dashboard, and
Postgres failover all still work — only `POST /deposits` needs a real test
key (see `.env.example`). First build compiles the Go binaries and takes a
few minutes; a repeat `up` takes under one.
...
<details>
<summary><strong>Requirements, port table, tests, and load test — click to expand</strong></summary>
...
### What comes up
`docker compose ps` should show 16 containers `Up` (the ones with a
healthcheck as `healthy`) and one, `kafka-init`, `Exited (0)` — that's
one-shot Kafka topic provisioning, not a failure.
...
breakdown of the bottlenecks — "Load testing" below, and the
[published report](https://claude.ai/code/artifact/b40504bd-656e-452a-bb32-3e4ec344bd26).
...
The async side, not pictured above: `auth-svc` and `transfers-svc` write
events to an outbox in the same Postgres transaction as the business change;
a relay publishes them to Kafka a second later, where `accounts-svc` (account
creation), `notifications-svc` (email), and the Gateway (WebSocket push) each
...
|---|---|---|
| **gateway** | JWT auth, HTTP routing, WebSocket push | every service (HTTP/gRPC) · Kafka (fans out events to WS connections) |
| **auth-svc** | users, sessions, profile, avatars | Redis (refresh tokens) · MinIO (avatar objects) · Kafka (outbox) |
| **accounts-svc** | account records, IBAN resolution, balance projection | ledger-svc (gRPC) · Kafka (consumer + outbox) |
| **ledger-svc** | the double-entry log and balances — no public API | Postgres only, called over internal gRPC |
| **transfers-svc** | transfers, deposits, withdrawals, reconciliation | accounts-svc, fraud-svc, ledger-svc (gRPC) · Stripe (REST + webhook) · Kafka |
| **fraud-svc** | rule-based transfer scoring | Postgres only, called by transfers-svc over gRPC |
| **notifications-svc** | transactional email, retry + DLQ | Kafka (consumer) · Mailpit (SMTP) |

Each service has its own short README — what it owns, its API, the events it
publishes and consumes: [gateway](gateway/README.md) ·
...
Seven problems the load test, the failover test, or a concurrency test
actually forced a decision on — not a list of technologies. Each links to
the real code, not a paraphrase of it.

**Zero-sum ledger under concurrency.** Two transfers hitting the same
account at once can both read "funds sufficient" before either writes,
pushing the balance negative. `executeTransfer` locks both accounts with
`SELECT ... FOR UPDATE` in a deterministic order (ascending `account_id`, to
rule out deadlock), before checking the balance — proven by a test that
fires 20 concurrent transfers at a 10,000-unit account and gets exactly 10
successes, never a negative balance.
[`ledger.go:343`](services/ledger-svc/ledger.go#L343) ·
[`ledger.go:312`](services/ledger-svc/ledger.go#L312) ·
[`TestExecuteTransfer_ConcurrentOverdraftPrevention`](services/ledger-svc/ledger_test.go#L655)

**Idempotency against a real race, not a hypothetical one.** A deposit or a
Stripe refund can be delivered more than once, by design — Kafka and Stripe
are both at-least-once. Posting the same operation twice would create money.
A `pg_advisory_xact_lock` on the operation's reference serializes concurrent
posts before either checks whether it already happened; a test fires 20
concurrent calls sharing one reference and gets exactly one ledger entry.
[`ledger.go:530`](services/ledger-svc/ledger.go#L530) ·
[`TestDeposit_ConcurrentSameReferenceDoesNotDoublePost`](services/ledger-svc/ledger_test.go#L965)

**Outbox instead of dual-write.** Writing a business row and publishing its
event as two separate steps means a crash between them either drops the
event or publishes one for a change that never committed. The event is
inserted into a table in the *same* Postgres transaction as the change; a
separate relay polls that table and publishes to Kafka afterward — the event
can't exist without the change, and can't be lost by a crash after it.
[`outbox.go:84`](pkg/outbox/outbox.go#L84) ·
[`relay.go:81`](pkg/outbox/relay.go#L81)

**Reconciling a transfer whose outcome is unknown.** A gRPC call to the
ledger can execute successfully while its response is lost to a timeout —
the caller genuinely cannot tell whether the money moved. Rather than guess,
the transfer stays `pending`; a worker asks the ledger directly, by
reference, and resolves it to `completed` or `failed` based on the real
answer — verified by killing `transfers-svc` mid-transfer with a
`SIMULATE_CRASH_AFTER_LEDGER_CALL` hook and watching the worker heal it.
[`reconcile.go:58`](services/transfers-svc/reconcile.go#L58) ·
[`reconcile.go:90`](services/transfers-svc/reconcile.go#L90) ·
[`transfer.go:608`](services/transfers-svc/transfer.go#L608)

**A saga across Stripe's boundary.** A deposit crosses two systems that
can't share a transaction — "Stripe took the card payment" and "the ledger
credited it" are separate facts, confirmed at different times. Two explicit
statuses (`succeeded` → `credited`, not one `completed`) keep that gap
visible; a background worker closes it, and a refund posts a *new* reversal
entry rather than deleting the original — the ledger is append-only, so
history and its correction both stay on the record.
[`deposit_reconcile.go:66`](services/transfers-svc/deposit_reconcile.go#L66) ·
[`webhook.go:207`](services/transfers-svc/webhook.go#L207)

**A signal, not data, over WebSocket.** Delivery order over a WebSocket
isn't guaranteed — a balance pushed straight into a message can arrive after
a newer one, or not at all, and the client has no way to tell a stale value
from a fresh one. So the socket only ever carries
`{"type":"balance.changed"}` — never an amount — forcing the client back to
an ordinary, authoritative HTTP request for the real number.
[`notify.go:55`](gateway/notify.go#L55)

**Postgres failover that loses nothing already confirmed.** A single
Postgres instance is a single point of failure, and naive failover can
promote a replica that never received a transaction the client was already
told succeeded. Patroni + etcd run in `synchronous_mode`: the leader can't
confirm a commit until a synchronous standby has it. A test kills the
container leader with `docker kill` under live load and checks every
transaction is fully paired afterward — measured at 23.6–25.4s of write
downtime, zero transactions lost, across four separate kills.
[`patroni.yml:99`](infra/patroni/patroni.yml#L99) ·
[`failover_test.go:424`](infra/failover/failover_test.go#L424)

## Tests and load

251 Go test functions across 37 files, plus one frontend test file — unit
and integration, run against a real Postgres, no mocked database. Every load
test run adds a second layer on top of HTTP status codes: 8 Postgres
invariant checks (double-entry balance, no duplicate idempotency keys, cache
matches the ledger log, …), re-run after every profile.

The headline number from the [full load test report](https://claude.ai/code/artifact/b40504bd-656e-452a-bb32-3e4ec344bd26):
**176.4 transfers/s** ceiling on distributed load, bottlenecked on
`ledger-svc`'s unconfigured 16-connection pool (441 of an 841ms request was
just waiting for a free connection); **31.5 transfers/s, flat**, on a single
hot account regardless of concurrency — a deliberate cost of the row lock
that keeps that account from ever going negative. **53,789 transfers, 87,888
ledger entries, 0 invariant violations** in the final run. Full breakdown,
all four bottlenecks, and the methodology caveats: "Load testing" below.

## Honest limitations

What follows is not done for real, or is a deliberate compromise, not
forgotten work. The framing is always the same: "I know and chose this,"
never "I didn't get to it."
