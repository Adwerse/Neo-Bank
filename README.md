...
Девять решений, которые стоит принимать осознанно, а не по умолчанию — что
...
MinIO — S3-совместимое объектное хранилище, поднятое для аватаров (бакет `avatars`, создаётся одноразовым `minio-init` при старте, тем же приёмом, что `kafka-init` создаёт топики). Пока в него ничего не пишет ни один сервис — сама загрузка файлов будет отдельным спринтом; в этом бакет только провизионируется. Креды — dev-only, как у Postgres. S3-совместимый API — не случайность, а причина выбора MinIO для дева: переезд в прод — это Cloudflare R2 или AWS S3 с тем же протоколом, то есть смена endpoint/креды в конфиге сервиса, который в итоге будет туда писать, а не переписывание кода.
...
`GET /profile` и `PATCH /profile` (через Gateway, `X-User-Id` из JWT — тот же паттерн, что `accounts-svc.GET /me`) отдают и обновляют `display_name` пользователя. Почему это auth-svc, а не отдельный сервис — мини-ADR «Профиль — в auth-svc, а не в отдельном profile-svc» выше. `avatar_key`/`avatar_updated_at` — колонки уже есть (миграция `000005`), но пока всегда `null`: сама загрузка аватара в MinIO — следующий спринт, здесь только место для неё подготовлено, чтобы форма ответа не менялась второй раз.
