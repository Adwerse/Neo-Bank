...
### Асинхронная часть — следующий спринт
Kafka-консьюмеры (`notifications-svc`, `accounts-svc`) и релей outbox в трейс пока не входят: контекст трейса не переезжает через сообщение, поэтому спаны консьюмера были бы корнями собственных трейсов. Это отдельная работа — заголовки Kafka, `PropagateFromMessage`, и решение о том, чем считать связь producer→consumer (child span или span link).
