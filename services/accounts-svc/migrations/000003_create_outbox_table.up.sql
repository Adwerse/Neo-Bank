-- accounts_outbox, not "outbox": every service shares one physical
-- "neobank" Postgres database, and transfers-svc already owns a table
-- literally named "outbox" — a bare name here would collide.
--
...
CREATE INDEX idx_accounts_outbox_unpublished ON accounts_outbox (published_at, id) WHERE published_at IS NULL;
